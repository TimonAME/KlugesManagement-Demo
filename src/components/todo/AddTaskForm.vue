...
const newTodo = ref(
  props.isEdit && props.todoToEdit
    ...
    : {
        title: '',
        description: '',
        dueDate: props.date ? props.date.split('T')[0] + 'T00:00:00.000Z' : null,
        userTodos: [],
      },
)
...
const displayedUsers = computed(() => {
  return users.value.filter(
    (user) => !newTodo.value.userTodos.some((todo) => todo.user.id === user.id),
  )
})
...
const fetchUsers = async () => {
  console.log('Fetching users disabled!')
  /*
  try {
...
const removeUser = (userId) => {
  ...
  newTodo.value.userTodos = newTodo.value.userTodos.filter((todo) => todo.user.id !== userId)
}
  ...
  <form @submit.prevent="handleSubmit" class="border border-gray-300 p-2 rounded-window">
    ...
    <div class="flex flex-row flex-wrap gap-2">
      ...
      <div class="relative w-fit">
        <div
          ...
          :class="
            selectedDate
              ? 'border-second-accent hover:border-main stroke-second-accent text-second-accent'
              : 'border-gray-300 hover:border-gray-400 stroke-secondary-text text-secondary_text'
          "
      ...
      <div class="relative" v-if="!props.isEdit">
        <button
          ...
          :class="[
            newTodo.userTodos.length === 0
              ? 'border-error-text hover:border-error-text animate-pulse'
              : '',
        ...
        >
          <svg
            class="w-4 h-4 stroke-secondary-text"
            viewBox="0 0 24 24"
            fill="none"
      ...
      <div class="relative" ref="userDropdownRef" v-if="!props.isEdit">
        <button
          ...
          :class="[
            showUserDropdown
              ? 'border-second-accent text-second-accent'
        ...
        >
          <svg
            class="w-4 h-4"
            :class="showUserDropdown ? 'stroke-second-accent' : 'stroke-secondary-text'"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
          ...
          <span
            class="text-sm"
            :class="showUserDropdown ? 'text-second-accent' : 'text-secondary_text'"
          >
            Person zuweisen
          </span>
        </button>
        ...
        >
          ...
          <div class="flex flex-row gap-2 items-center mb-2">
            ...
            />
            <button type="button" @click="showUserDropdown = false" class="flex-shrink-0">
              <svg
                class="w-6 h-6 stroke-secondary-text hover:stroke-error_text"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
          ...
          <div class="max-h-48 overflow-y-auto">
            <div v-if="isLoading" class="text-center py-2 text-gray-500">Laden...</div>
            ...
            <div v-else>
              ...
              <div v-if="displayedUsers.length === 0" class="text-gray-500 text-sm p-2 text-center">
    ...
    <div class="flex justify-end gap-2">
      ...
      <button
        ...
        :class="[!newTodo.title ? 'cursor-not-allowed opacity-50' : 'cursor-pointer']"
      >
        Update
      </button>
      <button
        ...
        :class="[
          !newTodo.title || newTodo.userTodos.length === 0
