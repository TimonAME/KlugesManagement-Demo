...
<template>
  ...
  <div v-if="activeView === 'suche'" class="relative mb-6">
    ...
    />
    <svg
      class="absolute left-2 top-1/2 transform -translate-y-1/2 w-5 h-5 stroke-gray-500"
      viewBox="0 0 24 24"
      fill="none"
    ...
    >
      ...
      <div class="flex items-center gap-4 group/item">
        <svg
          ...
          :class="activeView === 'erledigt' ? 'stroke-success-text' : 'stroke-icon-color'"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            :class="activeView === 'erledigt' ? 'hidden' : 'group-hover/svg:hidden block'"
          ...
          />
          <path
            :class="activeView === 'erledigt' ? 'block' : 'group-hover/svg:block hidden'"
          ...
          <path
            :class="
              activeView === 'erledigt' ? 'block' : 'stroke-icon-color hidden group-hover/svg:block'
            "
        ...
        <div class="flex flex-row gap-2" v-if="!todo.recentlyAdded">
          <svg
            v-if="activeView !== 'erledigt'"
            @click="openEditForm(todo)"
            class="w-6 h-6 stroke-icon-color invisible group-hover/item:visible hover:stroke-main"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
          ...
          <div class="relative invisible group-hover/item:visible">
            <svg
              ...
              class="w-6 h-6 stroke-icon-color hover:stroke-error-text transition-colors"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
            ...
            <div
              v-if="confirmDelete === todo.id"
              class="flex flex-row gap-2 absolute right-8 top-0"
            >
              <div
                @click="deleteConfirmation(true, todo)"
                class="p-2 bg-window_bg rounded-window shadow-window hover:bg-green-200 transition-colors cursor-pointer"
              >
                <svg
                  class="w-4 h-4 stroke-success-text"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
              ...
              <div
                @click="deleteConfirmation(false, null)"
                class="p-2 bg-window_bg rounded-window shadow-window hover:bg-red-200 transition-colors cursor-pointer"
              >
                <svg
                  class="w-4 h-4 stroke-error-text"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
  ...
  <div v-if="activeView !== 'erledigt' || activeView !== 'eingang'">
    <div class="p-4 cursor-pointer" @click="openTaskForm">
      <div class="flex items-center gap-4 group/add">
        ...
        <p class="text-sm text-secondary_text group-hover/add:text-second-accent">
          Aufgabe hinzufügen
        </p>
      </div>
    </div>

    <AddTaskForm v-if="isOpen" :is-edit="false" @add="handleAddTodo" @close="handleCloseForm" />
  </div>
</template>
