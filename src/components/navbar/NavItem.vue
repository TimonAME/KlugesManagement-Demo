...
const iconPath = computed(() => {
  return `${import.meta.env.BASE_URL}assets/icons/${props.iconName}.svg`
})
...
<template>
  <div v-if="navName === 'divider'" class="flex items-center justify-start w-full m-4">
    <div
      class="h-0.5 bg-icon-color rounded-full transition-all duration-150"
      :class="!isExpanded ? 'w-9' : 'w-44'"
    ></div>
  </div>
  ...
  >
    <div
      v-if="activePage.toLowerCase() === navName.toLowerCase()"
      class="absolute inset-0 bg-main opacity-15 z-10"
    ></div>
    <div class="relative z-20">
      <div
        ...
        :class="
          activePage.toLowerCase() === navName.toLowerCase()
            ? 'bg-main'
            : 'bg-icon-color group-hover:bg-main'
        "
        :style="{
    ...
    <span
      v-if="!isExpanded"
      class="fixed left-20 hidden group-hover:block duration-100 bg-window_bg text-primary_text px-2 py-1 rounded-window shadow-lg"
      >{{ navName }}</span
    >
    <span
      ...
      :class="[
        isExpanded
          ? 'opacity-100 w-[128px] duration-150 delay-75'
          : 'opacity-0 w-[0px] duration-0 text',
        activePage.toLowerCase() === navName.toLowerCase()
          ? 'text-main'
