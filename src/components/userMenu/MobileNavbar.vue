...
<template>
  ...
  <transition name="navbar">
    <div
      @click="openNavbar"
      v-if="showNavbar"
      class="fixed z-40 inset-0 bg-second_accent bg-opacity-90 flex flex-col justify-start items-end text-white"
    >
      <nav class="flex flex-col items-end gap-6 text-2xl mr-8 mt-18">
        <div
          @click="changePage(page.name.toLowerCase())"
        ...
        >
          ...
          <div
            v-if="page.icon"
            class="w-8 h-8 bg-white group-hover:bg-main"
            :style="{
              mask: `url(${getIconPath(page.icon)}) no-repeat center`,
              'mask-size': 'contain',
            }"
          ></div>
        </div>
