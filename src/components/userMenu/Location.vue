...
const otherLocations = ref(
  locations.value.filter((location) => location !== selectedLocation.value),
)
  ...
  >
    <div class="flex flex-col items-center group cursor-pointer">
      <div class="flex flex-row justify-between w-full gap-2 px-4 py-2">
        <span
          class="text-secondary_text text-md font-semibold group-hover:text-main transition-colors duration-100 w-16 truncate"
        >
          {{ selectedLocation }}
        </span>
        <svg
          class="stroke-icon_color group-hover:stroke-main transition-colors duration-100"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
