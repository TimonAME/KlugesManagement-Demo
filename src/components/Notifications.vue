...
const dummyNotifications = [
  {
    id: 1,
    message: 'Willkommen im Demo-System',
    isRead: false,
    date_created: { date: '2025-02-10T09:21:00' },
    notificationTags: [{ id: 1, name: 'System', hex_color: '#845EC2' }],
  },
  {
    id: 2,
    message: 'Neue Datei wurde erfolgreich hochgeladen',
    isRead: false,
    date_created: { date: '2025-02-08T15:45:00' },
    notificationTags: [
      { id: 2, name: 'Upload', hex_color: '#00C9A7' },
      { id: 4, name: 'Info', hex_color: '#0081CF' },
    ],
  },
  {
    id: 3,
    message: 'Demo Anfrage abgeschlossen',
    isRead: true,
    date_created: '2025-02-05T13:10:00',
    notificationTags: [{ id: 3, name: 'Success', hex_color: '#4CAF50' }],
  },
  {
    id: 4,
    message: 'Wartung in Kürze — bitte speichern',
    isRead: false,
    date_created: { date: '2025-02-04T18:22:00' },
    notificationTags: [{ id: 5, name: 'Wichtig', hex_color: '#FF6F61' }],
  },
]

const dummyTags = [
  { id: 1, name: 'System', hex_color: '#845EC2' },
  { id: 2, name: 'Upload', hex_color: '#00C9A7' },
  { id: 3, name: 'Success', hex_color: '#4CAF50' },
  { id: 4, name: 'Info', hex_color: '#0081CF' },
  { id: 5, name: 'Wichtig', hex_color: '#FF6F61' },
]

/* ------------------------------------------
   Persist from original component
------------------------------------------ */

const notifications = ref([])
const role = ref('DemoUser')
const allTags = ref([])
...
const fetchNotificationsByCategory = async (category) => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      if (category === 'unread') {
        notifications.value = dummyNotifications.filter((n) => !n.isRead)
      } else if (category === 'completed') {
        notifications.value = dummyNotifications.filter((n) => n.isRead)
      } else {
        notifications.value = dummyNotifications.filter((n) =>
          n.notificationTags.some((tag) => tag.name === category),
        )
      }
...
const checkNotification = async (id) => {
  if (
    notifications.value.find((n) => n.id === id).isRead ||
  ...
  setTimeout(() => {
    ...
    loadingNotifications.value = loadingNotifications.value.filter((x) => x !== id)
  }, 500)
}
</script>
        ...
        >
          ...
          <div class="flex items-center gap-4 group/item">
            <svg
              ...
              class="w-4 h-4 border p-0 rounded-window transition-colors duration-75 ease-in stroke-icon-color border-icon_color"
            ...
            >
              <path
                v-if="notification.isRead || loadingNotifications.includes(notification.id)"
                d="M20 6L9 17L4 12"
                stroke-width="2"
                stroke-linecap="round"
                stroke-linejoin="round"
            ...
            <div class="flex flex-col w-full">
              ...
              <div class="flex flex-row items-center gap-2">
                <svg
                  class="w-4 h-4 stroke-icon-color"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                ...
                <div
                  v-if="
                    new Date(notification.date_created.date).toLocaleString() === 'Invalid Date'
                  "
                ...
                <div v-else class="text-xs text-gray-500 mt-1">
                  {{ new Date(notification.date_created.date).toLocaleString() }}
                </div>
              </div>
            </div>
          </div>
