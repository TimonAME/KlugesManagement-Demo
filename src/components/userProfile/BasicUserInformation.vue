...
const currentUserRole = ref('ROLE_TEACHER')
const cantGoBack = ref(false)
const profilePicture = ref(null)
const profilePictureLoaded = ref(false)
const profilePictureError = ref(false)

// Fake loading for profile picture
onMounted(() => {
  cantGoBack.value =
    currentUserRole.value === 'ROLE_STUDENT' || currentUserRole.value === 'ROLE_GUARDIAN'
  setTimeout(() => {
    // Simuliere erfolgreiches Laden eines Profilbildes (oder Fehler)
    profilePicture.value = null // kein echtes Bild, somit fällt es zurück auf Initialen
    profilePictureLoaded.value = false
...
const formatRole = (roles) => {
  if (!roles || roles.length === 0) return 'Unbekannt'
  switch (roles[0]) {
...
const getInitialsAvatar = () => {
  if (!user.value.first_name || !user.value.last_name) return renderDefaultAvatar()

  const firstInitial = user.value.first_name.charAt(0).toUpperCase()
  const lastInitial = user.value.last_name.charAt(0).toUpperCase()

  return `<div class="flex items-center justify-center w-full h-full bg-[#a38cff51] rounded-window">
      <div class="text-3xl font-bold text-second-accent w-full h-full flex items-center justify-center">${firstInitial}${lastInitial}</div>
    </div>`
}

const renderDefaultAvatar = () => {
  return `<div class="flex items-center justify-center w-full h-full bg-gray-200 rounded-window">
      <svg width="64" height="64" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M20 21V19C20 17.9391 19.5786 16.9217 18.8284 16.1716C18.0783 15.4214 17.0609 15 16 15H8C6.93913 15 5.92172 15.4214 5.17157 16.1716C4.42143 16.9217 4 17.9391 4 19V21" stroke="#5F6368" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        <path d="M12 11C14.2091 11 16 9.20914 16 7C16 4.79086 14.2091 3 12 3C9.79086 3 8 4.79086 8 7C8 9.20914 9.79086 11 12 11Z" stroke="#5F6368" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
    </div>`
}
</script>
...
<template>
  <div class="flex flex-col justify-between h-full">
    <div>
      ...
      <div
        v-if="!cantGoBack"
        class="flex flex-row items-center gap-2 cursor-pointer mt-2 mb-4 group"
        @click="goToList()"
      >
        <svg
          class="stroke-icon-color w-6 h-6"
          viewBox="0 0 24 24"
          fill="none"
      ...
      <!-- personen daten -->
      <div class="mt-2">
        <h2 class="text-xl font-semibold text-second-accent">
          {{ user.first_name }} {{ user.last_name }}
        </h2>
        <div class="h-[1px] w-full rounded-full bg-gray-200 mt-1"></div>
        <div class="text-base font-semibold text-secondary_text mt-1">
      ...
      <!-- kontakt daten -->
      <div class="mt-6">
        <h2 class="text-lg font-semibold text-second-accent">Kontaktieren</h2>
        <div class="h-[1px] w-full rounded-full bg-gray-200 mt-1"></div>
        <div
          v-if="user.email"
        ...
        >
          <div class="flex flex-row gap-2 max-w-[calc(100%-32px)] overflow-hidden">
            <svg
              class="w-6 h-6 stroke-icon-color"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
          ...
          <svg
            class="stroke-icon-color w-6 h-6 group-hover:stroke-main opacity-0 group-hover:opacity-100 transition-opacity duration-100"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
      ...
      <div v-if="user.birthday" class="mt-6">
        <div class="text-lg font-semibold text-second-accent">Persönliche Daten</div>
        <div class="h-[1px] w-full rounded-full bg-gray-200 mt-1"></div>
        <div
