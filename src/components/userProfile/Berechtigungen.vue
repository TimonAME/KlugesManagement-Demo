...
onMounted(() => {
  // Demo: Lade Rolle nach kurzer Verzögerung
  setTimeout(() => {
    currentRole.value = 'ROLE_MANAGEMENT' // Demo-Standardrolle
    loading.value = false
  }, 1000)
})
...
const updateUserRole = async () => {
  ...
  try {
    // Demo: Simuliere Backend-Update
    await new Promise((resolve) => setTimeout(resolve, 1000))

    alert(
      `Rolle erfolgreich auf "${roles.find((r) => r.value === currentRole.value).label}" aktualisiert.`,
    )
  } catch (err) {
    error.value = 'Fehler beim Aktualisieren der Benutzerrolle'
  } finally {
