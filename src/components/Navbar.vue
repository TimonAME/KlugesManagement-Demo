...
<template>
  <nav
    class="stroke-icon-color hidden sm:flex h-svh bg-window_bg shadow-window min-h-screen overflow-hidden flex-col justify-between transition-all duration-300 select-none"
    :style="{ width: isExpanded ? '13rem' : '4.5rem' }"
  >
    <div class="h-[calc(100%-56px-56px-16px)] flex flex-col justify-start relative">
      ...
      >
        ...
        <span
          ...
          :class="
            isExpanded
              ? 'opacity-100 w-auto duration-150 delay-75 text-md'
              : 'text-xs opacity-0 w-0 duration-0'
          "
        >
          Your Company
        </span>
    ...
    <div class="flex flex-col items-center justify-center mb-4">
      <div
        class="w-full group flex items-center justify-start border-r-2 border-transparent hover:border-main transition-colors duration-100 cursor-pointer"
        @click="toggleNavbar"
      >
        <!-- Todo: change icon to single arrow -->
        <svg
          v-if="!isExpanded"
          class="w-6 h-6 my-4 ml-6 delay-100 stroke-icon_color group-hover:stroke-main transition-colors duration-100 rotate-45"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
        ...
        <span
          v-if="!isExpanded"
          class="absolute ml-20 hidden group-hover:block duration-100 bg-window_bg text-primary_text px-2 py-1 rounded-window shadow-lg"
          >Ausklappen</span
        >
        <span
          class="ml-4 group-hover:text-main transition-opacity text-primary_text"
          :class="
            isExpanded
              ? 'opacity-100 w-[128px] duration-150 delay-100'
              : 'opacity-0 w-[0px] duration-0 '
          "
        >
          Einklappen
        </span>
      ...
      <div
        ref="logoutButtonRef"
        @click="logout"
        :class="confirmLogout ? 'hover:border-error_text' : 'hover:border-main'"
        class="w-full cursor-pointer group flex items-center justify-start border-r-2 border-transparent transition-colors duration-100"
      >
        <svg
          :class="confirmLogout ? 'stroke-error_text' : 'stroke-icon_color group-hover:stroke-main'"
        ...
        >
          ...
          />
          <path
            :class="{ 'animate-arrow': confirmLogout }"
            d="M16 17L21 12L16 7"
            stroke-linecap="round"
            stroke-linejoin="round"
          />
        ...
        <span
          class="ml-4 transition-all"
          :class="[
            isExpanded
              ? 'opacity-100 w-[128px] duration-150 delay-100'
              : 'opacity-0 w-[0px] duration-0 ',
            confirmLogout
              ? 'text-error_text font-semibold'
              : 'text-primary_text group-hover:text-main',
          ]"
        >
          Abmelden
        </span>
